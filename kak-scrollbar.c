...
int get_flag_pos(int doc_pos) {
	...
	int flag_pos  = (int) ((window_height-1)*rel_pos + 0.5);
	return flag_pos;
}
...
int main(int argc, char **argv) {
	char *bar_str = argv[1];	// Arg 1: Scrollbar lines
	bar_format = argv[2];	// Arg 2: Scrollbar format
	char *desc_str = argv[3];	// Arg 3: Selection descs
	...
	window_start_pos = strtol(argv[7], NULL, 10);	// Arg 7: Window range
	...
	token = strtok_r(argv[7], " ", save);
	const long window_start_pos = strtol(token, NULL, 10);

	// Process scrollbar flags
	set_scrollbar_flags(bar_start, bar_end);

	// Process selection flags
	set_selection_flags(desc_str);
